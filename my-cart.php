...
if (isset($_POST['submit'])) {
	if (!empty($_SESSION['cart'])) {
		...
		<div style='background:#fff; padding:20px; border-radius:8px; text-align:center; width:300px;'>
			...
			<p style='margin:0 0 20px; color:#555;'>Your Cart hasbeen Updated</p>
...
if (isset($_POST['ordersubmit'])) {
	if (strlen($_SESSION['login']) == 0) {
		...
		foreach ($value as $qty => $val34) {
			$productQuery = mysqli_query($con, "SELECT productPrice FROM products WHERE id='$qty'");
			$productRow = mysqli_fetch_array($productQuery);
			$productPrice = $productRow['productPrice'];
			$totalPrice = $productPrice * $val34;
			mysqli_query($con, "insert into orders(userId,productId,quantity,total_amount) values('" . $_SESSION['id'] . "','$qty','$val34', '$totalPrice')");
...
if (isset($_POST['update'])) {
	...
	if ($query) {
		...
		<div style='background:#fff; padding:20px; border-radius:8px; text-align:center; width:300px;'>
			<h3 style='margin:0 0 10px; color:#333;'>Success</h3>
			<p style='margin:0 0 20px; color:#555;'>Billing Address has been updated</p>
			<button style='background:#28a745; color:#fff; border:none; padding:10px 20px; border-radius:5px; cursor:pointer;' onclick='closeAlert()'>OK</button>
		</div>
	</div>
	<script>
...
if (isset($_POST['shipupdate'])) {
	...
	if ($query) {
		...
		<div style='background:#fff; padding:20px; border-radius:8px; text-align:center; width:300px;'>
			<h3 style='margin:0 0 10px; color:#333;'>Success</h3>
			<p style='margin:0 0 20px; color:#555;'>Shipping Address has been updated</p>
			<button style='background:#28a745; color:#fff; border:none; padding:10px 20px; border-radius:5px; cursor:pointer;' onclick='closeAlert()'>OK</button>
		</div>
	</div>
	<script>
